...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", height: 28, padding: "0 12px", background: BT, color: B, fontSize: 11.5, fontWeight: 400, borderRadius: 999, letterSpacing: "0.05em", textTransform: "uppercase" }}>
      {children}
    </span>
  );
}
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                padding: 16,
                borderRadius: 14,
                background: index === 1 ? "rgba(0,87,255,0.08)" : "#F8FAFF",
                border: `1px solid ${index === 1 ? "#CBD4FF" : BD}`,
              }}
            >
              <div style={{ width: 42, height: 42, borderRadius: "50%", background: index === 1 ? B : "#EEF2FF", display: "grid", placeItems: "center", color: index === 1 ? "#fff" : B, fontWeight: 400 }}>
                0{index + 1}
              </div>
              <div>
                <p style={{ margin: 0, color: T1, fontSize: 16, fontWeight: 400 }}>{title}</p>
                <p style={{ margin: "4px 0 0", color: T2, fontSize: 13.5, lineHeight: 1.5 }}>{body}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <section id="employees" style={{ background: BG, padding: "clamp(48px,7vw,96px) 0" }}>
      <div style={W(PAGE_MAX)}>
        <div className="product-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12 }}>
              {points.map(p => (
                <li key={p} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <span style={{ width: 20, height: 20, borderRadius: "50%", background: BT, color: B, fontSize: 11, fontWeight: 400, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>✓</span>
          ...
          <div>
            <p style={{ fontSize: 12, color: "#596174", fontWeight: 400, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 10px" }}>Illustrative app preview</p>
            <div style={{ background: "#fff", borderRadius: 20, border: `1px solid ${BD}`, padding: 28, boxShadow: "0 24px 60px -20px rgba(20,30,60,0.14)", maxWidth: 360 }}>
              <p style={{ fontSize: 12, color: T3, fontWeight: 400, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 20 }}>This pay cycle</p>
              {[
                ...
                <div key={row.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderTop: `1px solid ${BD}` }}>
                  ...
                  <span style={{ fontSize: 15, fontWeight: 400, color: row.highlight ? B : T1 }}>{row.value}</span>
                </div>
              ))}
              <Link to="/#enquiry" style={{ display: "block", marginTop: 20, height: 44, background: B, color: "#fff", fontWeight: 400, fontSize: 14, borderRadius: 8, textAlign: "center", lineHeight: "44px", textDecoration: "none" }}>
                Withdraw now
              </Link>
            </div>
          </div>
    ...
    <section id="employers" style={{ background: "#fff", padding: "clamp(48px,7vw,96px) 0" }}>
      <div style={W(PAGE_MAX)}>
        <div className="product-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          {/* Mockup first */}
          <div>
            <p style={{ fontSize: 12, color: "#596174", fontWeight: 400, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 10px" }}>Illustrative dashboard preview</p>
            <div style={{ background: "#fff", borderRadius: 20, border: `1px solid ${BD}`, padding: 24, boxShadow: "0 24px 60px -20px rgba(20,30,60,0.14)" }}>
              <p style={{ fontSize: 13, fontWeight: 400, color: T1, margin: "0 0 16px" }}>Approval queue</p>
              {queue.map(r => (
                <div key={r.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderTop: `1px solid ${BD}` }}>
                  <div>
                    <p style={{ fontSize: 13.5, fontWeight: 400, color: T1, margin: 0 }}>{r.name}</p>
                    <p style={{ fontSize: 12, color: T2, margin: "2px 0 0" }}>{r.amount}</p>
                  </div>
                  <span style={{ fontSize: 11.5, fontWeight: 400, color: r.status === "Auto-approved" ? B : T3, background: r.status === "Auto-approved" ? BT : BG, borderRadius: 999, padding: "4px 12px" }}>{r.status}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    <section id="nbfc" style={{ background: BG, padding: "clamp(48px,7vw,96px) 0" }}>
      <div style={W(PAGE_MAX)}>
        <div className="product-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div>
            <p style={{ fontSize: 12, color: "#596174", fontWeight: 400, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 10px" }}>Illustrative portfolio preview</p>
            <div style={{ background: "#fff", borderRadius: 20, border: `1px solid ${BD}`, padding: 28, boxShadow: "0 24px 60px -20px rgba(20,30,60,0.14)", maxWidth: 360 }}>
              <p style={{ fontSize: 12, color: T3, fontWeight: 400, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 20 }}>Portfolio snapshot</p>
              {portfolioRows.map(r => (
                <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderTop: `1px solid ${BD}` }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 400, color: T1 }}>{r.value}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    <section id="disbursement" style={{ background: "#fff", padding: "clamp(48px,7vw,96px) 0" }}>
      <div style={W(PAGE_MAX)}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <h2 style={{ ...sectionTitleStyle, margin: "0 0 14px" }}>
          ...
          {steps.map(s => (
            <div key={s.n} style={{ borderRadius: 16, border: `1px solid ${BD}`, padding: "20px 18px", textAlign: "center" }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", border: `1.5px solid ${B}`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>
                <span style={{ fontSize: 11.5, fontWeight: 400, color: B }}>{s.n}</span>
              </div>
              <p style={{ fontSize: 14.5, fontWeight: 400, color: T1, margin: "0 0 6px" }}>{s.label}</p>
        ...
        <div className="product-3col" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
          {cards.map(c => (
            <div key={c.title} style={{ background: "#fff", borderRadius: 16, border: `1px solid ${BD}`, padding: "24px 20px" }}>
              <p style={{ fontSize: 15, fontWeight: 400, color: T1, margin: "0 0 10px" }}>{c.title}</p>
  ...
  return (
    <section style={{ background: "linear-gradient(180deg, #F2F5FF 0%, #FFFFFF 100%)", padding: "clamp(64px,8vw,112px) 0", textAlign: "center" }}>
      <div style={W(PAGE_MAX)}>
        ...
        <Link
          to="/#enquiry"
          style={{ display: "inline-flex", alignItems: "center", height: 52, padding: "0 32px", background: B, color: "#fff", fontWeight: 400, fontSize: 15.5, borderRadius: 8, textDecoration: "none", transition: "background 120ms ease, transform 120ms ease" }}
