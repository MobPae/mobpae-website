...
export function BenefitsSection() {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute right-0 top-24 h-96 w-96 rounded-full bg-[#ECEBFF]/80 blur-[110px]" />

      <div className="relative mx-auto max-w-7xl px-5 sm:px-6">
        <div className={`mx-auto max-w-3xl text-center reveal ${inView ? "in-view" : ""}`}>
          <p className="text-[11px] font-[700] uppercase tracking-[0.22em] text-[#5659D9]">Why teams choose MobPae</p>
          <h2 className="mt-5 text-[36px] font-[700] leading-[1] tracking-normal text-[#191A2E] lg:text-[48px]">
            Built for financial wellness without losing control.
          </h2>
        </div>

        <div className="mt-14 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <div className={`rounded-[36px] border border-[#E4E4EF] bg-white p-6 shadow-soft reveal delay-100 ${inView ? "in-view" : ""}`}>
            <div className="grid gap-6 md:grid-cols-[0.9fr_1.1fr] md:items-center">
              <div>
                <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-[#ECEBFF] text-[#5659D9]">
                  <Building2 size={24} />
                </div>
                <h3 className="mt-6 text-[28px] font-[700] leading-[1.05] tracking-normal text-[#191A2E]">
                  Employer control center
                </h3>
                <p className="mt-4 text-[14px] leading-[1.8] text-[#62657A]">
                  Set advance percentage, max limits, KYC rules, bank verification and approval flow from admin settings.
                </p>
              </div>
              <div className="rounded-[30px] border border-[#9FD7DD] bg-[#E9F6F6] p-5 text-[#191A2E]">
                <div className="mb-5 flex items-center justify-between">
                  <p className="text-[13px] font-[700]">Policy settings</p>
                  <span className="rounded-full bg-white px-3 py-1 text-[10px] font-[700] text-[#5659D9]">Active</span>
                </div>
        ...
        <div className={`theme-card-grid mt-5 grid gap-5 lg:grid-cols-3 reveal delay-300 ${inView ? "in-view" : ""}`}>
  ...
  return (
    <div className="mb-4 last:mb-0">
      <div className="mb-2 flex items-center justify-between gap-3">
        <span className="text-[11px] font-[600] text-[#62657A]">{label}</span>
        <span className="text-[12px] font-[700] text-[#191A2E]">{value}</span>
      </div>
      <div className="h-2 rounded-full bg-white">
        <div className="h-full rounded-full bg-gradient-to-r from-[#A5A7FF] to-[#7679FF]" style={{ width }} />
      </div>
    </div>
  );
}
...
function FeatureTile({ icon, title, copy }: { icon: React.ReactNode; title: string; copy: string }) {
  return (
    <div className="rounded-[32px] border border-[#E4E4EF] bg-white p-6 shadow-soft">
      ...
      <h3 className="mt-5 text-[21px] font-[700] tracking-normal text-[#191A2E]">{title}</h3>
      <p className="mt-3 text-[13.5px] leading-[1.75] text-[#62657A]">{copy}</p>
    </div>
  );
}
...
function InsightCard({ title, value, label }: { title: string; value: string; label: string }) {
  return (
    <div className="relative overflow-hidden rounded-[32px] border border-white p-6 shadow-soft">
      <div className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-[#ECEBFF]" />
      <div className="relative">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#7679FF] text-white">
          <Gauge size={19} />
        </div>
        <p className="mt-5 text-[13px] font-[700] uppercase tracking-[0.12em] text-[#B7B9C7]">{title}</p>
        <p className="mt-2 text-[34px] font-[700] leading-none tracking-normal text-[#191A2E]">{value}</p>
