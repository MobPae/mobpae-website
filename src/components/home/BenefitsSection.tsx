...
export function BenefitsSection() {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute right-0 top-24 h-96 w-96 rounded-full bg-[#F0EDFF]/80 blur-[110px]" />
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-6">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <div className={`rounded-[36px] border border-[#E5E7EB] bg-white p-6 shadow-soft reveal delay-100 ${inView ? "in-view" : ""}`}>
            <div className="grid gap-6 md:grid-cols-[0.9fr_1.1fr] md:items-center">
              ...
              <div className="rounded-[30px] border border-[#9FD7DD] bg-[#E9F6F6] p-5 text-[#111827]">
                <div className="mb-5 flex items-center justify-between">
                  <p className="text-[13px] font-[700]">Policy settings</p>
                  <span className="rounded-full bg-white px-3 py-1 text-[10px] font-[700] text-[#4E32CA]">Active</span>
                </div>
  ...
  return (
    <div className="mb-4 last:mb-0">
      <div className="mb-2 flex items-center justify-between gap-3">
        <span className="text-[11px] font-[600] text-[#6B7280]">{label}</span>
        <span className="text-[12px] font-[700] text-[#111827]">{value}</span>
      </div>
      <div className="h-2 rounded-full bg-white">
        <div className="h-full rounded-full bg-gradient-to-r from-[#B8ACFF] to-[#5B3CE3]" style={{ width }} />
      </div>
    </div>
  );
}
...
function InsightCard({ title, value, label }: { title: string; value: string; label: string }) {
  return (
    <div className="relative overflow-hidden rounded-[32px] border border-white p-6 shadow-soft">
      <div className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-[#F0EDFF]" />
      <div className="relative">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#5B3CE3] text-white">
          <Gauge size={19} />
        </div>
