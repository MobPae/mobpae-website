      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-7xl px-5 sm:px-6">
          <div className="mb-10 max-w-3xl">
            <p className="text-[11px] font-[700] uppercase tracking-[0.22em] text-emerald-700">Who this is for</p>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {audiences.map((item) => (
              <article key={item.title} className="rounded-[32px] border border-emerald-100 bg-[#F6F9F8] p-6 shadow-[0_12px_34px_rgba(15,23,42,0.035)]">
                <div className="flex h-13 w-13 h-[52px] w-[52px] items-center justify-center rounded-3xl bg-emerald-100 text-emerald-700">
                  {item.icon}
                </div>
                <h3 className="mt-6 text-[21px] font-[700] tracking-normal text-slate-950">{item.title}</h3>
      ...
      <section className="bg-[#F6F9F8] py-20">
        <div className="mx-auto max-w-7xl px-5 sm:px-6">
          <div className="grid gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
            <div>
              <p className="text-[11px] font-[700] uppercase tracking-[0.22em] text-emerald-700">MVP onboarding</p>
              <h2 className="mt-4 text-[36px] font-[700] leading-[1] tracking-normal text-slate-950 lg:text-[48px]">
            ...
            <div className="rounded-[36px] border border-emerald-100 bg-white p-5 shadow-soft">
              <div className="grid gap-3">
                {timeline.map((item, index) => (
                  <div key={item} className="flex items-center gap-4 rounded-[24px] bg-[#F6F9F8] px-4 py-4">
                    <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-[12px] font-[700] text-white">
                      {index + 1}
                    </span>
                    <p className="text-[15px] font-[700] text-slate-950">{item}</p>
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-6">
          <div className="rounded-[38px] border border-emerald-100 bg-slate-950 p-7 text-white shadow-[0_28px_80px_rgba(6,78,59,0.18)] lg:p-9">
            <div className="grid gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
              <div>
                <p className="text-[12px] font-[700] uppercase tracking-[0.16em] text-emerald-200/70">Trust positioning</p>
                ...
                <a href="/contact" className="mt-7 inline-flex h-12 items-center justify-center gap-2 rounded-full bg-white px-6 text-[14px] font-[700] text-slate-950 transition hover:-translate-y-1 hover:bg-emerald-50">
...
function TrustRow({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-white/8 px-4 py-4">
      ...
      <p className="text-[14px] font-[700] text-white/82">{text}</p>
    </div>
  );
}
