...
export function FaqSection() {
  ...
  const inputClass =
    "w-full rounded-xl border border-[#E4EAE7] bg-[#F6F9F8] px-4 py-3 text-[13px] text-[#101828] placeholder-[#94a3b8] outline-none transition focus:border-[#0F8F72] focus:ring-2 focus:ring-[#E9F8F3]";

  return (
    <section id="faq" className="relative overflow-hidden bg-[#F6F9F8] py-20">

      {/* Blobs */}
      <div className="pointer-events-none absolute left-0 top-0 h-56 w-56 rounded-full bg-[#0F8F72]/5 blur-[70px]" />
      <div className="pointer-events-none absolute right-0 bottom-24 h-56 w-56 rounded-full bg-[#E9F8F3]/60 blur-[70px]" />
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <div className="mb-12 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-[#E9F8F3] bg-[#F0FAF7] px-4 py-2">
            ...
            <span className="text-[11px] font-[700] uppercase tracking-[0.2em] text-[#0F8F72]">FAQ & Contact</span>
          </div>
          <h2 className="mt-5 text-[36px] font-[600] tracking-normal leading-[1.1] text-[#101828] lg:text-[42px]">
            Everything you need to know
          </h2>
          <div className="mt-4 mx-auto h-0.5 w-16 bg-[#0F8F72]" />
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_400px] lg:items-start">
            ...
            {faqs.map((faq, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor: isOpen ? "#E9F8F3" : "#E4EAE7",
                ...
                >
                  ...
                  >
                    <span className={`text-[14.5px] font-[700] transition-colors ${isOpen ? "text-[#101828]" : "text-slate-700"}`}>
                      {faq.q}
                    </span>
                    <div className={`ml-4 flex-shrink-0 transition-colors ${isOpen ? "text-[#0F8F72]" : "text-[#94a3b8]"}`}>
                      {isOpen ? <Minus size={16} /> : <Plus size={16} />}
                    </div>
                  </button>
                  {isOpen && (
                    <div className="px-6 pb-5">
                      <p className="text-[13.5px] leading-[1.8] text-[#667085]">{faq.a}</p>
                    </div>
                  )}
                </div>
              );
          ...
          <form
            ...
            className="rounded-2xl border border-[#E9F8F3] bg-white p-7 shadow-soft lg:sticky lg:top-24"
          >
            <div className="flex items-center gap-4 mb-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#F0FAF7]">
                <Mail size={18} className="text-[#0F8F72]" />
              </div>
              <div>
                <h3 className="text-[16px] font-[600] text-[#101828]">Get in touch</h3>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                style={{ background: "linear-gradient(135deg, #0F8F72, #047857)", boxShadow: "0 18px 40px rgba(16,185,129,0.24)" }}
              >
...
function FormField({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="mb-1.5 block text-[12px] font-[700] text-[#101828]">{label}</label>
      {children}
