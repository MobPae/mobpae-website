  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-40 top-20 h-[440px] w-[440px] rounded-full bg-[#F0EDFF]/50 blur-[120px]" />
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-6">
        ...
        <div className={`reveal-scale delay-150 overflow-hidden rounded-[46px] border border-[#DED7FF] bg-white shadow-[0_30px_80px_rgba(91,60,227,0.12)] ${inView ? "in-view" : ""}`}>
          <div className="relative grid lg:grid-cols-[0.8fr_1.02fr_0.88fr]">
            <div className="pointer-events-none absolute inset-0">
              <div className="absolute left-[30%] top-[-20%] h-[500px] w-[500px] rounded-full bg-[#F0EDFF] blur-[110px]" />
              <div className="absolute bottom-[-24%] right-[-8%] h-[520px] w-[520px] rounded-full bg-[#E9F6F6] blur-[110px]" />
              <div className="absolute inset-0 bg-[linear-gradient(rgba(91,60,227,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(91,60,227,0.045)_1px,transparent_1px)] bg-[size:44px_44px]" />
            </div>

            <div className="relative z-10 border-b border-[#E5E6EE] bg-[#F8F9FC]/80 p-5 sm:p-7 lg:border-b-0 lg:border-r lg:p-8">
              <p className="text-[10px] font-[700] uppercase tracking-[0.2em] text-[#4E32CA]">Product tour</p>
                  ...
                  return (
                    <button
                      ...
                      className={`group flex min-h-[76px] items-center gap-3 rounded-[22px] border px-3 text-left transition-all sm:px-4 ${
                        isActive
                          ? "border-[#B9BBFF] bg-[#F0EDFF] text-[#111827] shadow-[0_12px_30px_rgba(91,60,227,0.1)]"
                          : "border-[#E5E6EE] bg-white text-[#6B7280] hover:border-[#DED7FF] hover:bg-[#F7F7FF] hover:text-[#111827]"
                      }`}
                    >
                      <span className={`grid h-10 w-10 flex-shrink-0 place-items-center rounded-2xl ${isActive ? "bg-[#5B3CE3] text-white" : "bg-[#E9F6F6] text-[#4E32CA]"}`}>
                        <Icon size={18} />
                      </span>
            ...
            <div className="relative z-10 flex min-h-[650px] items-center justify-center overflow-hidden bg-[#F7F7FF]/70 px-5 py-10 sm:px-8">
              <div className="absolute h-[480px] w-[480px] rounded-full border border-[#DED7FF] bg-[#F0EDFF]/60" />
              <div className="absolute h-[380px] w-[380px] rounded-full border border-white" />
              <PhoneFrame>
                <ProductScreen active={active} />
              </PhoneFrame>
              <div className="absolute bottom-7 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full border border-[#DED7FF] bg-white/90 px-4 py-2 backdrop-blur-xl">
                {productStages.map((stage, index) => (
                  <button
                    key={stage.name}
                    type="button"
                    ...
                    onClick={() => setActive(index)}
                    className={`h-2 rounded-full transition-all ${active === index ? "w-8 bg-[#5B3CE3]" : "w-2 bg-[#D5D7E2]"}`}
                  />
                ))}
              </div>
            </div>
                ...
                <div className="mt-7 grid gap-3">
                  {selected.points.map((point) => (
                    <div key={point} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/10 px-3 py-3">
                      <span className="grid h-8 w-8 flex-shrink-0 place-items-center rounded-xl bg-white text-[#5B3CE3]">
                        <Check size={14} />
                      </span>
...
function PhoneFrame({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative z-20 w-full max-w-[318px] rounded-[42px] border border-[#DED7FF] bg-white p-2.5 shadow-[0_28px_70px_rgba(91,60,227,0.18)]">
    ...
    <div className="product-screen-enter">
      ...
      <div className="mt-4 rounded-[22px] border border-[#E5E7EB] bg-white p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-[#F8F9FC] px-3 py-2 text-[10px] font-[700] text-[#4E32CA]">Track</span>
        </div>
        <div className="mt-5 flex items-center gap-2">
          {[true, true, false, false].map((done, index) => <span key={index} className={`h-2 flex-1 rounded-full ${done ? "bg-[#5B3CE3]" : "bg-[#E5E7EB]"}`} />)}
        </div>
      </div>
    </div>
  );
      ...
      <div className="mt-4 rounded-[22px] border border-[#E5E7EB] bg-white p-4">
        ...
        <div className="mt-5 space-y-4">
          {["Submitted", "Employer approval", "Admin review", "Payout"].map((label, index) => (
            <div key={label} className="flex items-center gap-3"><span className={`grid h-7 w-7 place-items-center rounded-full ${index < 2 ? "bg-[#5B3CE3] text-white" : "bg-[#F8F9FC] text-[#E5E7EB]"}`}><Check size={12} /></span><span className="text-[11px] font-[700] text-[#6B7280]">{label}</span></div>
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="product-screen-enter">
      ...
      <div className="flex items-center gap-3 rounded-[22px] border border-[#E5E7EB] bg-white p-4">
        <div className="grid h-12 w-12 place-items-center rounded-full bg-[#F0EDFF] text-[#4E32CA]"><UserRound size={20} /></div>
...
function ProfileRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="flex items-center gap-3 rounded-[20px] border border-[#E5E7EB] bg-white p-3"><span className="grid h-9 w-9 place-items-center rounded-2xl bg-[#F8F9FC] text-[#4E32CA]">{icon}</span><span className="min-w-0 flex-1 text-[11px] font-[700] text-[#111827]">{label}</span><span className="rounded-full bg-[#F8F9FC] px-2 py-1.5 text-[9px] font-[700] text-[#4E32CA]">{value}</span></div>;
}
