  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-40 top-20 h-[440px] w-[440px] rounded-full bg-[#f1f5ff]/50 blur-[120px]" />
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-6">
        <div className="mx-auto mb-12 max-w-[900px] text-center">
          <p className={`reveal text-[11px] font-[700] uppercase tracking-[0.22em] text-[#214be6] ${inView ? "in-view" : ""}`}>
            Inside MobPae
          </p>
        ...
        <div className={`reveal-scale delay-150 overflow-hidden rounded-[46px] border border-[#dce5ff] bg-white shadow-[0_30px_80px_rgba(49,94,255,0.12)] ${inView ? "in-view" : ""}`}>
          <div className="relative grid lg:grid-cols-[0.8fr_1.02fr_0.88fr]">
            <div className="pointer-events-none absolute inset-0">
              <div className="absolute left-[30%] top-[-20%] h-[500px] w-[500px] rounded-full bg-[#f1f5ff] blur-[110px]" />
              <div className="absolute bottom-[-24%] right-[-8%] h-[520px] w-[520px] rounded-full bg-[#E9F6F6] blur-[110px]" />
              <div className="absolute inset-0 bg-[linear-gradient(rgba(49,94,255,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(49,94,255,0.045)_1px,transparent_1px)] bg-[size:44px_44px]" />
            </div>

            <div className="relative z-10 border-b border-[#E5E6EE] bg-[#F8F9FC]/80 p-5 sm:p-7 lg:border-b-0 lg:border-r lg:p-8">
              <p className="text-[10px] font-[700] uppercase tracking-[0.2em] text-[#214be6]">Product tour</p>
                  ...
                  return (
                    <button
                      ...
                      className={`group flex min-h-[76px] items-center gap-3 rounded-[22px] border px-3 text-left transition-all sm:px-4 ${
                        isActive
                          ? "border-[#B9BBFF] bg-[#f1f5ff] text-[#111827] shadow-[0_12px_30px_rgba(49,94,255,0.1)]"
                          : "border-[#E5E6EE] bg-white text-[#6B7280] hover:border-[#dce5ff] hover:bg-[#F7F7FF] hover:text-[#111827]"
                      }`}
                    >
                      <span className={`grid h-10 w-10 flex-shrink-0 place-items-center rounded-2xl ${isActive ? "bg-[#315eff] text-white" : "bg-[#E9F6F6] text-[#214be6]"}`}>
                        <Icon size={18} />
                      </span>
            ...
            <div className="relative z-10 flex min-h-[650px] items-center justify-center overflow-hidden bg-[#F7F7FF]/70 px-5 py-10 sm:px-8">
              <div className="absolute h-[480px] w-[480px] rounded-full border border-[#dce5ff] bg-[#f1f5ff]/60" />
              <div className="absolute h-[380px] w-[380px] rounded-full border border-white" />
              <PhoneFrame>
                <ProductScreen active={active} />
              </PhoneFrame>
              <div className="absolute bottom-7 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full border border-[#dce5ff] bg-white/90 px-4 py-2 backdrop-blur-xl">
                {productStages.map((stage, index) => (
                  <button
                    key={stage.name}
                    type="button"
                    ...
                    onClick={() => setActive(index)}
                    className={`h-2 rounded-full transition-all ${active === index ? "w-8 bg-[#315eff]" : "w-2 bg-[#D5D7E2]"}`}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="brand-dark-card relative z-10 flex flex-col justify-center border-t border-white/10 p-7 text-white lg:border-l lg:border-t-0 lg:p-9">
              <div key={selected.name} className="product-copy-enter">
                <p className="text-[10px] font-[700] uppercase tracking-[0.2em] text-[#b6c6ff]">{selected.eyebrow}</p>
                ...
                <div className="mt-7 grid gap-3">
                  {selected.points.map((point) => (
                    <div key={point} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/10 px-3 py-3">
                      <span className="grid h-8 w-8 flex-shrink-0 place-items-center rounded-xl bg-white text-[#315eff]">
                        <Check size={14} />
                      </span>
...
function PhoneFrame({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative z-20 w-full max-w-[318px] rounded-[42px] border border-[#dce5ff] bg-white p-2.5 shadow-[0_28px_70px_rgba(49,94,255,0.18)]">
      <div className="overflow-hidden rounded-[33px] bg-[#F8F9FC]">
        <div className="flex h-9 items-center justify-between px-6 text-[10px] font-[700] text-[#111827]">
          <span>9:41</span>
          <span>•••  5G</span>
        </div>
...
function ScreenHeading({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <div className="mb-5">
      <p className="text-[9px] font-[700] uppercase tracking-[0.16em] text-[#214be6]">{eyebrow}</p>
  ...
  return (
    <div className="product-screen-enter">
      <div className="mb-5 flex items-center gap-3">
        <div className="grid h-10 w-10 place-items-center rounded-full bg-[#f1f5ff] text-[#214be6]"><UserRound size={18} /></div>
      ...
      <div className="mt-4 rounded-[22px] border border-[#E5E7EB] bg-white p-4">
        <div className="flex items-center justify-between">
          <div><p className="text-[9px] font-[700] uppercase tracking-[0.14em] text-[#B7B9C7]">Live request</p><p className="mt-1 text-[17px] font-[700] text-[#111827]">Request journey</p></div>
          <span className="rounded-full bg-[#F8F9FC] px-3 py-2 text-[10px] font-[700] text-[#214be6]">Track</span>
        </div>
        <div className="mt-5 flex items-center gap-2">
          {[true, true, false, false].map((done, index) => <span key={index} className={`h-2 flex-1 rounded-full ${done ? "bg-[#315eff]" : "bg-[#E5E7EB]"}`} />)}
        </div>
      </div>
    </div>
  );
...
function AdvanceScreen() {
  return (
    <div className="product-screen-enter">
      ...
      <div className="mt-4 rounded-[22px] border border-[#E5E7EB] bg-white p-4">
        <p className="text-[13px] font-[700] text-[#111827]">Repayment preview</p>
...
function TrackScreen() {
  return (
    <div className="product-screen-enter">
      ...
      <div className="rounded-[26px] border border-[#dce5ff] bg-[#f1f5ff] p-5 text-[#111827] shadow-[0_16px_38px_rgba(49,94,255,0.12)]">
        <div className="flex items-start justify-between gap-2"><div><p className="text-[10px] font-[600] text-[#6B7280]">Live request · #DEMO</p><p className="mt-3 text-[34px] font-[700] leading-none tracking-normal">₹5,000</p></div><span className="rounded-full bg-white px-3 py-2 text-[10px] font-[700] text-[#214be6]">Pending</span></div>
      ...
      <div className="mt-4 rounded-[22px] border border-[#E5E7EB] bg-white p-4">
        <p className="text-[13px] font-[700] text-[#111827]">Request journey</p>
        <div className="mt-5 space-y-4">
          {["Submitted", "Employer approval", "Admin review", "Payout"].map((label, index) => (
            <div key={label} className="flex items-center gap-3"><span className={`grid h-7 w-7 place-items-center rounded-full ${index < 2 ? "bg-[#315eff] text-white" : "bg-[#F8F9FC] text-[#E5E7EB]"}`}><Check size={12} /></span><span className="text-[11px] font-[700] text-[#6B7280]">{label}</span></div>
          ))}
        </div>
      </div>
    </div>
...
function ProfileScreen() {
  return (
    <div className="product-screen-enter">
      ...
      <div className="flex items-center gap-3 rounded-[22px] border border-[#E5E7EB] bg-white p-4">
        <div className="grid h-12 w-12 place-items-center rounded-full bg-[#f1f5ff] text-[#214be6]"><UserRound size={20} /></div>
        <div><p className="text-[14px] font-[700] text-[#111827]">Arjun Sharma</p><p className="mt-1 text-[10px] font-[700] text-[#B7B9C7]">Northstar Retail</p></div>
      </div>
...
function BrandCard({ label, amount, sub }: { label: string; amount: string; sub: string }) {
  return (
    <div className="rounded-[26px] border border-[#dce5ff] bg-[#f1f5ff] p-5 text-[#111827] shadow-[0_16px_38px_rgba(49,94,255,0.12)]">
      <div className="flex items-start justify-between gap-2"><p className="text-[11px] font-[600] text-[#6B7280]">{label}</p><span className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-2 text-[10px] font-[700] text-[#214be6]"><ShieldCheck size={12} /> Member</span></div>
      ...
      <div className="mt-5 h-2 rounded-full bg-white"><div className="h-full w-[86%] rounded-full bg-[#315eff]" /></div>
    </div>
  );
}

function MiniMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[17px] border border-[#E5E7EB] bg-white p-3"><p className="text-[9px] font-[600] text-[#B7B9C7]">{label}</p><p className="mt-2 text-[12px] font-[700] text-[#111827]">{value}</p></div>;
}

function DataRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return <div className="flex items-center justify-between gap-3"><span className="text-[#8D90A3]">{label}</span><strong className={strong ? "text-[#111827]" : "text-[#6B7280]"}>{value}</strong></div>;
}

function ProfileRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="flex items-center gap-3 rounded-[20px] border border-[#E5E7EB] bg-white p-3"><span className="grid h-9 w-9 place-items-center rounded-2xl bg-[#F8F9FC] text-[#214be6]">{icon}</span><span className="min-w-0 flex-1 text-[11px] font-[700] text-[#111827]">{label}</span><span className="rounded-full bg-[#F8F9FC] px-2 py-1.5 text-[9px] font-[700] text-[#214be6]">{value}</span></div>;
}
